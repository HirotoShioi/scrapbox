...
import           Constructors (blockQuote, bold, bulletPoint, codeNotation,
                               ...
                               markdown, noStyle, scrapText, simpleText,
...
everythingIs :: Block
everythingIs = header 2 [simpleText "📝 Everything is editable"]

-- "\tClick on any line and start typing to edit. "
clickOn :: Block
clickOn = bulletPoint 1 $ scrapText
    [noStyle [simpleText "Click on any line and start typing to edit. "]]

-- "\t\tPress tab at the beginning of a line to indent and add a bullet point."
pressTab :: Block
pressTab = bulletPoint 2 $ scrapText
    [noStyle [simpleText "Press tab at the beginning of a line to indent and add a bullet point."]]

-- " Highlight text to make it a [new link], [* bold], [- and] [/ more]."
highlightText :: Block
highlightText = bulletPoint 1 $ scrapText
...
hereIs :: Block
hereIs = header 2 [simpleText "🎯 Here is where it gets interesting "]

-- "\tClick a [new link] to create a new page with that title and open it.",
clickNewLink :: Block
clickNewLink = bulletPoint 1 $ scrapText [noStyle [clickA, newLink, toCreate]]
  where
...
pagesThat :: Block
pagesThat = bulletPoint 2 $ scrapText
    [noStyle [simpleText "Pages that are directly linked or two steps away from the current page \
    \will be displayed."]]

-- "\tSee images, videos, and external links added inside `[` brackets`]` on the page",
seeImages :: Block
seeImages = bulletPoint 1 $ scrapText
...
whatCan :: Block
whatCan = header 1 [simpleText "What can you put in a Scrapbox project?"]
...
-- "[* For example]",
forExample :: Block
forExample = bulletPoint 1 $ scrapText [noStyle [simpleText "For example"]]
...
letsSay :: Block
letsSay = bulletPoint 1 $ scrapText [ noStyle [letsSayText, sitePlan, toStart, socialMedia, period]]
  where
...
onceYouGot :: Block
onceYouGot = header 2 [simpleText "📌 Once you've got the basics, here are ways to dig deeper and \
...
seeAList = document $ scrapText [noStyle [seeAListText, thingsYouCando, space]]
  where
    ...
    thingsYouCando = link (Just "Things you can do") "https://scrapbox.io/help/Things%20you%20can%20do"
    space          = simpleText " "

-- " \tIncludes more syntax, inviting team members, and creating profiles",
includesMore :: Block
includesMore = bulletPoint 1 $ scrapText
    [noStyle [simpleText "Includes more syntax, inviting team members, and creating profiles"]]

-- "\tSee some [https://scrapbox.io/help/examples Example projects] ",
seeSome :: Block
seeSome = bulletPoint 1 $ scrapText [noStyle [seeSomeText, exampleProjects, space]]
  where
    seeSomeText     = simpleText "See some "
    exampleProjects = link (Just "Example projects") "https://scrapbox.io/help/exampless"
    space           = simpleText " "

-- " \tIncludes a SaaS startup, design agency, and more",
includesSaas :: Block
includesSaas = bulletPoint 2 $ scrapText [noStyle [simpleText "Includes a SaaS startup, design agency, and more"]]


-- "\tSee [https://scrapbox.io/help/ How-tos and support] ",
howTos :: Block
howTos = bulletPoint 1 $ scrapText [noStyle [see, howToSimpleText, space]]
  where
    see             = simpleText "See "
    howToSimpleText = link (Just "How-tos and support") "https://scrapbox.io/help/"
    space           = simpleText " "

-- " \tFor detailed instructions and answers to FAQs",
forDetails :: Block
forDetails = bulletPoint 2 $ scrapText [noStyle [simpleText "For detailed instructions and answers to FAQs"]]

-- "[* We would love to hear any questions or feedback you may have]",
weWouldLove :: Block
weWouldLove = header 1 [simpleText "We would love to hear any questions or feedback you may have"]
...
pleaseLet = document $ scrapText [noStyle [pleaseLetText, twitter, andText, faceBook]]
  where
    ...
    twitter       = link (Just "twitter") "https://twitter.com/scrapboxapp"
    faceBook      = link (Just "facebook") "https://facebook.com/scrapboxapp"
